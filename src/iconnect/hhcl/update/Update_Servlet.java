...
public class Update_Servlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		ResultSet Res = null;
		/*
		 * try { // conn =dataSource.getConnection(); connection = (java.sql.Connection)
		 * session .getAttribute("ConnectionObj"); } catch (Exception e1) { // TODO
		 * Auto-generated catch block e1.printStackTrace(); }
		 */
		final Logger logger = Logger.getLogger(Update_Servlet.class);
		long start = System.currentTimeMillis();
			
		connection=null; 
			try {
				try {
					connection = ConnectionpoolFilter.Call();
			...
			logger.info("Login Servlet Main  - welcome " +connection );
		...
		try {
			ConnectionpoolFilter.close(connection, Res,"Update_Servlet",false);
		} catch (SQLException e) {
