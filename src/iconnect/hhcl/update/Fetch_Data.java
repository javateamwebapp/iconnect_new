...
public class Fetch_Data extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * try { connection=(java.sql.Connection)session.getAttribute("ConnectionObj");
		 * } catch (Exception e1) { e1.printStackTrace(); }
		 */
	      final Logger logger = Logger.getLogger(Fetch_Data.class);
	      ...
	      try {
				ConnectionpoolFilter.close(connection, rs,"Fetch_Data",true);
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
