...
public class Attendance_flexi extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * try { // conn =DataSource_Cls.getInstance().getConnection();
		 * conn=(java.sql.Connection)session.getAttribute("ConnectionObj"); try{
		 * conn.setAutoCommit(false); }catch(Exception err){ err.printStackTrace(); } }
		 * catch (Exception e1) { // TODO Auto-generated catch block
		 * e1.printStackTrace(); }
		 */
		
		final Logger logger = Logger.getLogger(Attendance_flexi.class);
     ...
     Res=null;
		...
		if(Routing.equalsIgnoreCase("Att_Request")){
			...
			out.write(""+values.toString()+"");  
			 ...
			 try {
					ConnectionpoolFilter.close(conn, Res,"Attendance_flexi",false);
				} catch (SQLException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
