...
public class SMSGatewayCenterAPI{
	...
	public static synchronized  Map CallSmsData(String Mobile,String Otp){
		...
		try{
			...
			data += "&text="+URLEncoder.encode(" HETERO HEALTHCARE & AZISTA iConnect verification is "+Otp+". This OTP is Valid for one trxn.or 15 mins only.Do not Share it with anyone" ,"UTF-8");
			URL url = new URL("http://login.smsgatewayhub.com/api/mt/SendSMS?" + data);
			
			
			System.out.println("the Mobile sms gate way::"+url);
		...
		System.out.println(buffer.toString()+"Its Sms buffer");
